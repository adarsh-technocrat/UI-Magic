import { listOfDynamicComponent } from '../app/data';
...
figma.ui.onmessage = (msg) => {
  ...
  if (msg.type === 'generate-ui') {
    ...
    generateOriginalComponentUIWithAnimationForMobileAndDesktop();
  }
};
...
const createDuplicateOfTheComponentFromAnotherPage = async () => {
  ...
  if (targetPage) {
    for (let i = 0; i <= dynamicComponents.length; i++) {
      const duplicatedElement = targetPage.children.find((node) => node.name === dynamicComponents[i].name);
      const cloneElement = duplicatedElement.clone();
      cloneElement.x += 100;
      cloneElement.y += 100;
      currentPage.appendChild(cloneElement);
      figma.viewport.scrollAndZoomIntoView([cloneElement]);
      await delay(1000);
    }
...
//   animationContainer.resize(360, 49);
...
//     dynamicComponent.resize(360, dynamicComponents[idx].height);
...
//     mobileFrame.resizeWithoutConstraints(360, currentComponentHeight + 40);
//     animationContainer.y = currentComponentHeight; // Moving the Position of the UI
//     await delay(1000);
//   }

//   animationContainer.remove();
//   mobileFrame.resizeWithoutConstraints(360, currentComponentHeight);
// };

const generateOriginalComponentUIWithAnimationForMobileAndDesktop = async () => {
  let currentComponentHeight = 0;
  ...
  //  Get The Targeted Screen

  const targetPage = figma.root.children.find((page) => page.name === 'UI Designing');

  // Generating Dynamic Components
  for (let idx = 0; idx < listOfDynamicComponent.length; idx++) {
    const componentNode = targetPage.findChild(
      (node: FrameNode) => listOfDynamicComponent[idx].component === node.name
    );
    const cloneComponent = componentNode.clone();
    cloneComponent.y = currentComponentHeight;
    cloneComponent.x = 0;
    mobileFrame.appendChild(cloneComponent);
    console.log('Clone properties:', cloneComponent.x, cloneComponent.y, cloneComponent.width, cloneComponent.height);
    currentComponentHeight += cloneComponent.height;
    mobileFrame.resizeWithoutConstraints(360, currentComponentHeight + 40);
    animationContainer.y = currentComponentHeight; // Moving the Position of the UI
    await delay(1000);
  }
